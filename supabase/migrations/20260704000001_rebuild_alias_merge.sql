-- Migration: rebuild_incentive_weekly_stats — alias-merge fix
-- Applied live 2026-07-04 via execute_sql MCP.
-- Change: GROUP BY COALESCE(employee_id, technician_name) so techs with
--         multiple raw JC names (e.g. "JATIN - HOODI" + "JAUTIN SAHOO - HSR LAYOUT"
--         both mapped to WRCT0164) are counted as one row, not two.
-- Also: MODE() WITHIN GROUP (ORDER BY technician_name) picks the dominant display name.
-- Definition below is verbatim from pg_get_functiondef() on the live DB.
...
BEGIN
  ...
  agg AS (
    SELECT
      -- Group by employee_id when available (merges aliases); fall back to technician_name
      COALESCE(l.employee_id, l.technician_name)                                       AS group_key,
      -- Display name: pick the name with the most JCs (MODE = most frequent)
      MODE() WITHIN GROUP (ORDER BY l.technician_name)                                 AS tech_name,
      l.week_start,
    ...
    GROUP BY COALESCE(l.employee_id, l.technician_name), l.week_start
  ),
  calc AS (
    SELECT
