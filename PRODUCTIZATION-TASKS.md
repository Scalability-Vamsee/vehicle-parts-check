...
*Last updated: 2026-07-29 (cont.) — RSA-KML Central Cluster gap fixed (eef3f24); incentive.html PWA mobile fixes + SW v2 + attendance biller-proxy from 20 Jul (e0b6794). Earlier same day: fw-map MCU 60A default; incentive Analytics L0 tiles week-select fix; RSA_BIKES now 7 entries — +Akhil +Nagendra (Sreeranga's 27-Jul request) +Tara Chand +Karan Luitel, ⚠️ Assigned-filter UI gap (RSA-BIKES row); rsa.html BLR zone cluster overlay + Chaikin smoothing; mcu_bikes_live view + anon revoke. RSA Admin Module tasks 6–14 defined. Prior: 2026-07-26 jc-failure-alert v6 + jc-history-sync v12.*
...
|---|---------|--------|-------|
| FW1 | DIU / MCU 60A toggle | ✅ LIVE | Exclusive toggle added to fw-map.html. `mcu_bikes_live` Supabase view (rfd_violations_cache regen_60a_check_needed=true → GPS + rider PII). Hub name via CASE WHEN map (14 hubs). Pushed commits 13b0d5a + b43cb0b. |
| FW2 | MCU 60A default on load | ✅ LIVE b43cb0b | `_fwMode='mcu'`; active class on MCU toggle button. |
| FW3 | Anon PII revoke (fw_bikes_live + mcu_bikes_live) | ✅ Supabase live | `REVOKE SELECT FROM anon` on both views. Rider name/phone authenticated-only. |

---

## RSA Admin Module (2026-07-29 — planned)

| # | Feature | Status | Notes |
|---|---------|--------|-------|
| RSA-KML | BLR zone cluster overlay on Warroom map | ✅ LIVE 298b3c3 · gap fix eef3f24 | 6 KML polygons (Central/YPR/RR Nagara/Bellandur/Hoodi/Hebbal) with Chaikin smoothing (3 iter). Off by default, Layers panel toggle. **2026-07-29 (cont.):** Central Cluster had gaps vs. all 4 neighbors — KML-verified root cause (Central only had 4 corners vs. neighbors' 7–9), snapped to neighbors' shared junction vertices (4→7 points), no self-intersection, original corners preserved. |
| RSA-BIKES | RSA_BIKES roster — Akhil, Nagendra, Tara Chand, Karan Luitel added | ✅ LIVE ff944eb · ⚠️ filter UI gap | Both `fw-map.html` + `rsa.html` arrays now 7 entries (was 3). Akhil + Nagendra added per Sreeranga's 27-Jul mapping request; Tara Chand + Karan Luitel same session. All 4 show as map dots. **Gap:** `rsa.html` Assigned-filter checkboxes/dropdown still only list Nishanth/Pavan/Bhoja — the 4 new techs' tickets can't be isolated via that filter. Small follow-up, see Fleetpro-context.md §2026-07-29. Superseded long-term by RSA-3 (DB-driven vehicle assignment). |
| RSA-1 | `rsa-admin` permission + DB schema | ⬜ | New `rsa-admin` feature key in group_features. Tables: `rsa_roster_template`, `rsa_roster_overrides`, `rsa_vehicle_assignments`, `rsa_spare_ledger`. Foundation for all RSA admin features. |
| RSA-2 | Roster UI — tech/hub assignment, weekly overrides, leave toggle | ⬜ BLOCKED on RSA-1 | Default template loads each week; admin can override per week. Leave/WO boolean on override row hides tech from warroom map layer. Historic roster preserved. |
| RSA-3 | Vehicle management UI — DB-driven RSA_BIKES | ⬜ BLOCKED on RSA-1 | `rsa_vehicle_assignments` table replaces hardcoded RSA_BIKES array in fw-map.html + rsa.html. Admin UI to assign/unassign bikes to techs. |
| RSA-4 | RSA Vehicle Health — swap freq + test drive + SOC alert | ⬜ | Swap freq (no swap in 3 days), test drive (bike GPS must move 500m+ from assigned hub in 3 days via cron), SOC <50% → email alert to admin+warroom+tech via Resend. |
| RSA-5 | Spare Ledger — tech-level parts inventory | ⬜ | `rsa_spare_ledger (tech_id, hub_id, part_name, entry_type, quantity, entry_date, jc_id, entered_by, note)`. Closing count = initial + adjustments. Manual entry (admin) first; JC auto-consumption Phase 2. Date range filter. |
| RSA-6 | GPS: Intellicar + BAAS merge | ⬜ | Blocked on data availability check. `bike-location-sync` v9 already writes both sources to `bike_location_cache`. Need to verify Intellicar coverage vs BAAS and update picking logic if needed. |
| RSA-7 | RSA Tech PWA — shell, auth, duty start/end, home screen | ✅ LIVE (pending push) | `rsa-tech.html` + `rsa-tech-manifest.json` + `rsa-tech-sw.js`. `rsa_duty_log` table live (migration 20260731000005). Auth = permissions-first, feature key `tech-app`. Home: tech profile card, hub name + Google Maps nav, Start/End Duty button (GPS-stamped), today's duty log timeline. Tabs: Home (active) + Tickets/FW Map (disabled, pending RSA-8/9). Tech lookup by `email ILIKE` from `rsa_technicians`. |
| RSA-8 | RSA Tech PWA — ticket flow + spare ledger + Warroom ticket view | ⬜ | RSA Tickets tab in PWA: in-app buzzer on new ticket. Accept → Navigate (maps) → Reach Location (photo) → Close (disposition + spare used). **Also:** ticket view panel in rsa.html Warroom (detail slide-in). Spare ledger integration on Close. |
| RSA-9 | RSA Tech PWA — FW Map integration | ⬜ | FW Map tab in PWA: MCU/DIU pending bikes near tech location. Tap → FW form/ticket. |
...
| IA2 | Analytics tab in v8/incentive.html (LIVE data) | ✅ LIVE | Built + **pushed 2026-07-09 (commit 5b16277)**. Superadmin/incentive-admin gated; live fetch from `incentive_weekly_stats` + `hub_productivity_daily`. ⚠️ Verify on live tab: if yellow banner shows, run `CREATE POLICY "auth read" ON hub_productivity_daily FOR SELECT TO authenticated USING (true);` **2026-07-16 (commit 900d95b): fixed pooled JC/tech/day** — `anPooled` was dividing full-week JCs by only the attendance days synced to `hub_productivity_daily` (W6 had 2/7 → jpt showed 23.04). Now projects tech-days = (Σ per-hub avg techs/day) × days-in-week (matches reference `buildChart0`); W6 → 6.58. Added "N/7 att" label when attendance incomplete. **Drill-down field hotfix (commit 1eb7b71):** modal queries `technician_name` (normalized) not `technician_name_raw` (was 0 rows). **Drill-down IST + chronic badge (commit c05aaab):** billed/comeback times shown MM/DD HH:MM UTC→IST; `weight≥2` rows get amber "×N chronic" badge + row bg. **2026-07-17 (ce63938):** removed Cohort card; two-line week labels on void/guardrail/city charts; hub-breakdown selected-week bars green(improved)/red(declined) + value labels + JC-count tooltip. **Payout display fix (c2828b2):** all 6 payout spots read frozen `payout_amount` from DB (calcPayout null-fallback) — frozen weeks were recomputing (W3 ₹17,900→₹14,725, W4 ₹15,750). **2026-07-18 Hyd attendance (416507f):** Fathenagar/Hyderabad flat **3.5 techs/day for W1–W5**, real from W6 (`HYD_ASSUMED_TECHS`/`HYD_REAL_FROM='2026-07-06'`); verified via `RRR/OOS_Productivity_Jun1_Jul8_2026.xlsx` (W5 = only 3 day-rows Jul 3–5, avg 2.67 partial tail; W6 = 3.5). **Miyapur→Fathenagar merge (41d851a):** alias + hub-breakdown normalized-key keying → one Hyderabad hub. ⚠️ 3.5 + merge live only in Analytics JS, not the data layer (see IA8). |
...
| IA2_26 | HYD hybrid fill + anPooled sparse-week fallback | ✅ LIVE (pushed 767796e) | **2026-07-26:** `HYD_ASSUMED_TECHS` reverted to 3.5 (was 4); removed blanket `HYD_REAL_FROM` cutoff. All 3 analytics paths (`anChartTechs`, `anPooled`, `jptOf`) now use real `hub_productivity_daily` per day, fall back to 3.5 only for days with no real Hyd row. `anPooled` fallback for sparse current-week attendance: hubs with JCs but no synced attendance fall back to prior week's td/days rate — fixes W8 projection collapsing to 4.63. |
| IA8 | Harden attendance pipeline (hub_productivity_daily) | ⬜ NEW (2026-07-16), mitigated 2026-07-29 | **Root cause of chronic partial weeks** (W6 = 2/7 days; W4/W5 stuck at 6/7 even weeks later) — **escalated 2026-07-29: the ops attendance Google Sheet stopped syncing entirely after 20 Jul** (see IA10 below for the JS-layer mitigation shipped same day). Loader EXISTS: per migration `20260708000002` table comment, **`oos-productivity-sync` GAS loads OOS sheet → `hub_productivity_daily` daily @ 9:30 AM IST** ("tab date = data date + 1 day"). That GAS is NOT mirrored in this repo (Apps Script lives in Google); `RRR/Productivity_Consolidator.gs` builds the `MASTER_Productivity` tab, and `RRR_MetabaseSync_v2.gs` has `supabaseUpsert()` but only for incentive tables, not productivity. So the gap is upstream/silent-failure, not a missing loader. Durable fix = **audit + harden the existing GAS**: (1) confirm its time trigger is installed & firing (execution log) — now a live outage as of 20 Jul, not just a lag; (2) determine whether it reads `MASTER_Productivity` or the daily tabs (needs GAS source); (3) add failure/empty-data alerting (silent skip → invisible "N/7 att"); (4) upstream — reduce manual daily-tab data entry (form / rolling sheet). Schema: `hub_productivity_daily(date, hub_name, technician_available numeric(4,1), …)` UNIQUE(date,hub_name). **BLOCKED until the `oos-productivity-sync` Apps Script source + trigger config are provided.** **Also add at data layer:** Miyapur→Fathenagar hub mapping (same hub; currently merged only in Analytics-tab JS via `AN_HUB_ALIAS`, commit 41d851a) + Hyderabad's missing early-week attendance (the Analytics tab band-aids this with a flat 3.5 for W1–W5 per 416507f). |
| IA10 | Attendance biller-proxy (sheet stopped syncing 20 Jul) | ✅ LIVE (commit e0b6794, 2026-07-29) | The `oos-productivity-sync` GAS (IA8) stopped syncing entirely after 2026-07-20 — not just lagging, dead. `ATT_SHEET_STALE_FROM='2026-07-20'`: from that date, Analytics always uses `anBillerRate` (distinct technicians billing ≥1 non-void JC per hub per day, from `incentive_jc_log`, averaged across the week) as the tech-headcount denominator instead of `hub_productivity_daily`. Before that date, real attendance stays primary; proxy only fills zero-attendance hub-weeks (same pattern as the pre-existing Hyderabad 3.5/day assumption, IA2). Chart shows real (light) vs proxy (dashed amber) series separately. Does not replace IA8 — if/when the GAS is restored, real attendance resumes as primary automatically (date-gated, not code-gated). |
| IA11 | Incentive PWA mobile fixes + service worker v2 | ✅ LIVE (commit e0b6794, 2026-07-29) | 7-item mobile audit: Trend/Leaderboard tables `overflow-x:auto`; compact trend-table dates ("15–21 Jun", drill-down keeps full format); larger drill-down search-icon tap target (≥44px on mobile); `color-scheme:light` (stops forced-dark inversion — trade-off: no real dark mode, flagged as separate future work); KPI values use `clamp(1.6rem,6vw,2.6rem)`. `incentive-sw.js` v2: `incentive.html` dropped from precache (deploys too often — a stale precached copy would silently serve on flaky network); fallback cache now refreshes on every successful live fetch; `SW_UPDATED` postMessage → non-blocking reload banner on new deploy. Skipped: manifest icon maskable safe-zone (needs visual inspection, low-risk per audit). |
...
| H4 | jc-failure-alert: `intrip` column blank in emails | ✅ deployed · ⏸ git | 2026-07-26. `dms_api_call_json` (JSON with embedded newlines/commas) shattered the old regex/line-split CSV parser — `intrip` mapped to `undefined`. Fixed with RFC 4180 compliant `parseCSV`. Also changed labels: `intrip=true` → "🔴 RUNNING REPAIR", `intrip=false` → "GENERAL SERVICES (Repossessed)". Deployed as v6 via MCP. **Source at `supabase/functions/jc-failure-alert/index.ts` — needs /tmp-clone push.** See Fleetpro-context.md §2026-07-26. |
| H5 | jc-history-sync stalled (page showing 22 Jul, OOM crash) | ✅ deployed · ⏸ git | 2026-07-26. Metabase card grew to 372k rows / 46MB → edge function hit `WORKER_RESOURCE_LIMIT` (HTTP 546) silently. v12 fixes: streaming reader (no full-payload memory) + 90-day incremental delete/reinsert. Metabase card `a2c3e48b` updated by user to filter last 90 days. Full 372,053-row seed done from bash via temp `SECURITY DEFINER` fn (2025-04-01→2026-07-24). Cron (jobs 21+41) now maintains 90-day window every 2–3h. **Deployed v12 via MCP — needs /tmp-clone push.** See Fleetpro-context.md §2026-07-26. |
| H6 | Anil Kumar login failure (duplicate email in incentive_technicians) | ✅ Supabase applied | 2026-07-26. Two rows shared `anilkumarbogle@gmail.com` — Anil Kumar Banaswadi (correct) + Nishanth CK Hoodi (stale; removed from Gsheet). Auth `.single()` threw on two matches → "not registered" on login. Fix: deleted Nishanth CK row (`id 3b04e665-7989-4816-8f87-14b94fe0f1ee`). |
| H7 | Jun 1/Jun 8 void% wrong in incentive_weekly_stats | ✅ Supabase applied | 2026-07-26. Pre-launch weeks `is_frozen=true`; early void rows were `is_void=false` because the 30-day card had rolled past Jun 1–14 comebacks by mid-June sync start. The 90-day card synced correct void data into `incentive_jc_log` on 2026-07-24. Fix: unfreeze → rebuild → re-freeze. Jun 1 → 9.5% (was 0.0%), Jun 8 → 7.8% (was 0.6%). Jun 15+ unchanged. Backfill SQL at `sql/rrr/RRR_Incentive_VoidBackfill_EarlyJune.sql` (commit 975ed30). |
...
| A3 | Page Analytics (`admin-analytics.html`) | ✅ LIVE | Superadmin-gated; reads `page_events`; `logPageView()` on fw-map/incentive/trace-ho. **2026-07-02**: Sync Jobs panel fully wired — `admin-cron` edge fn ACTIVE (verify_jwt=true) + migration `20260702000001` applied (admin_cron_list/set_schedule/set_active RPCs). **2026-07-08:** tabbed layout (Page Analytics / Sync Jobs), cron group headers, status pills, per-job last-run history via new RPC `admin_cron_last_runs` (migration `20260708000001`) — all pushed & live (stale ⚠️ UNPUSHED note cleared 2026-07-19). **2026-07-19:** `storageKey:'fleetpro_session'` + favicon → logo.jpg (833b6db). ⚠️ Confirm `admin_cron_last_runs` RPC + migration `20260708000001` + admin-cron redeploy are live — now reachable since storageKey fix lets admins sign in without the magic-link re-prompt. ⚠️ Confirm `page_events` INSERT RLS + extend logging to remaining pages. |
| A4 | RFD Check (`rfd-check.html`) | ✅ LIVE | Search-first admin tool: enter a reg → verdict **RFD CLEAR / INCOMPLETE** + per-check status (IoT Glue, MCU FW 60A, JC Billed) + context (rental/prev/ops-last status, activated by/at, RFD validation, JC dates). Catches bikes force-activated by **direct DB `vehicle_status='active'`** (Altmobility BaaS pings don't reach us via API) without clearing IoT/MCU or billing the JC. Backend (Cowork/MCP, all live): `rfd_violations_cache` (24 cols, RLS auth-read) ← `rfd-check-sync` edge fn v1 (Metabase Q `0cfdcf89`, strips test bikes, 500-row batches) on **pg_cron job 42** (`0 */2 * * *`). Frontend v1 table → **search-first refactor** + sidebar Service→Admin + index tile: commits `d382ea3`→`4cc4dd5`; feature key `rfd-check` at `b7ac7a1` (grant per group; superadmin bypass). **2026-07-19 redesign (881612e):** maintenance-style dark hero + Try: hint chips (random IoT/MCU-pending bikes) + sync bar (last updated + next run); magic-link auth replaces password; `storageKey:'fleetpro_session'` (root-cause of magic-link re-prompt); hub name from `rental_locations` hubMap (was "Hub 29"); **4-state JC** (Billed ✅ / Not Opened ⚠️ amber / Not Billed ⚠️ / No JC ➖); labels: "Before Activation" + "Last Ops Action" + "JC Stage" + "JC Last Updated"; favicon → logo.jpg. ⚠️ `rfd-check-sync` `verify_jwt=false`; Metabase query ~9,937 rows (JC-not-billed dominates) → a future list view should default to IoT/MCU-pending. |
