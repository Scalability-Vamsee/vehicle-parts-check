...
| Page Analytics + Sync Jobs | `v8/admin-analytics.html` | ✅ · ⚠️ 2026-07-08 unpushed | Page-view analytics + Sync Jobs panel (admin-cron, migration 20260702000001). **2026-07-08 (local, unpushed):** tabbed layout (Page Analytics / Sync Jobs) + cron group headers + per-job last-run history. |
| Admin-cron edge fn | `supabase/functions/admin-cron/index.ts` | ✅ · ⚠️ 2026-07-08 change unpushed | Superadmin-only: list/edit pg_cron jobs. verify_jwt=true. **2026-07-08 (local):** adds last-run history via RPC `admin_cron_last_runs` (migration `20260708000001_admin_cron_last_runs.sql`, NEW). Needs push + admin-cron redeploy + migration run. |
